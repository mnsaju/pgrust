...
pub fn fc_prsd_start(_flinfo: Option<&mut FmgrInfo>, fcinfo: &mut Fcinfo) -> PgResult<Datum> {
    ...
    // SAFETY: internal-arg contract above; str_ptr/len come from the caller's
    // fcinfo args and the buffer outlives the parser.
    let prs = unsafe { parser::tparser_init(fcinfo.result_mcx(), str_ptr, len.max(0) as usize) }?;
...
fn parse_rows(fcinfo: &Fcinfo) -> PgResult<SrfRows> {
    ...
    let mcx = fcinfo.result_mcx();
    let data = txt.data();
    // SAFETY: data (a text varlena's bytes) outlives prs within this call.
    let mut prs = unsafe { parser::tparser_init(mcx, data.as_ptr(), data.len()) }?;
