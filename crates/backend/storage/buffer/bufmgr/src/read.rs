...
fn complete_read_local(
    ...
    // SAFETY: blk is a live BLCKSZ buffer page image.
    let verified = unsafe { page_is_verified(blk, blkno, piv_flags, Some(&mut failed_checksum)) };
...
fn AsyncReadBuffers(
    ...
    // SAFETY: ret_ptr is a live &mut derived pointer, valid for this call.
    let ioh = match unsafe {
        aio_core::pgaio_io_acquire_nb(Some(resowner::CurrentResourceOwner()), ret_ptr)?
    } {
        Some(ioh) => ioh,
        None => {
            aio_core::pgaio_submit_staged()?;
            unsafe { aio_core::pgaio_io_acquire(Some(resowner::CurrentResourceOwner()), ret_ptr)? }
        }
    };
...
///
/// # Safety
/// `page` must be a live, readable BLCKSZ buffer page image.
pub unsafe fn page_is_verified(
    page: *const u8,
    blkno: BlockNumber,
    flags: u32,
    ...
    if pd_upper != 0 {
            ...
            && transam_xlog_seams::data_checksums_enabled::call()
        {
            ...
            if checksum != pd_checksum {
                ...
                if let Some(p) = checksum_failure_p {
                    *p = true;
                }
            }
        }
