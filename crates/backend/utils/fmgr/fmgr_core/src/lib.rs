...
static BUILTINS: [FmgrBuiltin; FMGR_NBUILTINS] = build_builtins();
    ...
    for t in tables {
        for b in *t {
            let live = fmgr_isbuiltin(b.foid)
                .is_some_and(|x| x.func as usize != builtin_not_ported as *const () as usize);
...
pub fn fmgr_isbuiltin(id: Oid) -> Option<&'static FmgrBuiltin> {
    match FMGR_BUILTIN_OID_INDEX.lookup(&FMGR_BUILTINS, id) {
        Some(b) if b.func as usize == builtin_not_ported as *const () as usize => extra_builtin(id).or(Some(b)),
        Some(b) => Some(b),
...
pub fn fmgr_info_from_builtin_into(fbp: &FmgrBuiltin, function_id: Oid, finfo: &mut FmgrInfo) {
    let fbp = if fbp.func as usize == builtin_not_ported as *const () as usize {
    ...
    let fn_addr = match row.prolang {
        INTERNAL_LANGUAGE_ID => {
            ...
            match fmgr_lookup_by_name(&prosrc) {
                ...
                Some(fbp) if fbp.func as usize == builtin_not_ported as *const () as usize => {
