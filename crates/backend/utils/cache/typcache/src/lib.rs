...
// lookup_type_cache's opclass-resolution chain mirrors C's special-cased
// operator checks (array/record/range element support) verbatim: each
// branch nulls the resolved OID for a different reason, so the bodies match
// even though the conditions test unrelated special-case operators.
