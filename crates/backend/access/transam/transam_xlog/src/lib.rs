...
pub const fn INSERT_FREESPACE(endptr: XLogRecPtr) -> usize {
    if endptr.is_multiple_of(XLOG_BLCKSZ as u64) {
        0
    } else {
...
pub fn XLogBytePosToRecPtr(bytepos: u64) -> XLogRecPtr {
    ...
    let seg_offset = if bytesleft < (XLOG_BLCKSZ - SizeOfXLogLongPHD) as u64 {
        ...
        XLOG_BLCKSZ as u64 + fullpages * XLOG_BLCKSZ as u64 + bytesleft + SizeOfXLogShortPHD as u64
    };
