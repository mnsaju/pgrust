...
impl<'mcx> CbScanDescData<'mcx> {
    ...
    pub fn meta_agg_scan(
        ...
        let fold_sums = zq.is_none_or(|z| z.keep_nonzero);
    ...
    fn cond_slot(&mut self) -> Option<(&crate::condcache::RgEntry, u32)> {
        ...
        if self.staged_rows > self.window_rows || !self.staged_lo.is_multiple_of(self.window_rows) {
            return None;
        }
        let part = self.part.as_ref()?;
...
fn analyze_extract_task(
    ...
    for r_ref in refs.iter().take(task.hi).skip(task.lo) {
        debug_assert_eq!(r_ref.0, task.rg);
        let r = r_ref.1 as usize - g * GRANULE_ROWS;
        for &c in needed_idx {
            let d = cds[c as usize].datum(r);
            if coltypes[c as usize].is_text() {
                while !out.bytes.len().is_multiple_of(4) {
                    out.bytes.push(0);
                }
