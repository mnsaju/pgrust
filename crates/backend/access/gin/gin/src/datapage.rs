...
use crate::postinglist::{
    ginCompressPostingList, ginMergeItemPointers, ginPostingListDecodeAllSegments, seg_first, seg_size,
};
            ...
            if !self.is_build {
                while leaf.lastleft > 0 {
                    ...
                    if leaf.segs[li].action != GIN_SEGMENT_DELETE {
                        ...
                        if ((leaf.lsize - segsize) as i64 - (leaf.rsize + segsize) as i64) < 0 {
                            break;
                        }
