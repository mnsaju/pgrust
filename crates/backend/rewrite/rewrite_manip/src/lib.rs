...
pub fn copy_node_list<'mcx>(mcx: Mcx<'mcx>, list: &NodeList<'mcx>) -> PgResult<NodeList<'mcx>> {
    ...
    copied.as_list().expect("List round trip").clone_in(mcx)
}
        ...
        match node.node_tag() {
            ...
            NodeTag::T_CurrentOfExpr => {
                if self.min_sublevels_up == 0 {
                    return Err(internal("cannot push down CurrentOfExpr"));
                }
                Ok(false)
            }
...
fn rte_at<'mcx>(rtable: &NodeList<'mcx>, varno: i32) -> &'mcx RangeTblEntry<'mcx> {
    rtable
...
pub fn ReplaceVarsFromTargetList_list<'mcx>(
    ...
    out.as_list().expect("List").clone_in(mcx)
}
    ...
    match node.node_tag() {
        ...
        NodeTag::T_CurrentOfExpr => {
            ...
            if cexpr.cvarno == ctx.target_varno as u32 && ctx.sublevels_up == 0 {
                return Err(
                    feature_not_supported("WHERE CURRENT OF on a view is not implemented"),
                );
            }
            Ok(None)
        }
