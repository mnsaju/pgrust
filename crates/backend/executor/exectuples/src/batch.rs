...
//
// The column loops below index several parallel arrays (atts, plan.offs,
// soa's flattened per-column SoA storage) by the same column index with
// arithmetic beyond simple indexing — an iterator rewrite would obscure the
// hot-path SoA layout more than it would clarify it.
