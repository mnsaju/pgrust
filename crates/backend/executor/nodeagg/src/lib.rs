...
use ::execexpr::{
    exec_build_agg_projection_info_subplans, exec_build_agg_qual_subplans, exec_eval_expr, exec_project, exec_qual, AggBind, AggOrderedSpec,
...
fn init_pertrans_sort<'mcx>(
    ...
    assert!(
        num_trans_inputs < MAX_ORDERED_TRANS_ARGS,
...
pub unsafe fn agg_state_is_shared(fcinfo: &::types_fmgr::FunctionCallInfoBaseData) -> bool {
    ...
    match unsafe { fcinfo.agg_state_node() } {
        Some(node) => node.current_agg().is_none_or(|(_, shared)| shared),
        None => true,
    }
}
