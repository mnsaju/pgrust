...
// Ok(Some((src, trg))) parsed a rule; Ok(None) a blank/comment line; Err a
// syntax error (-1/-2, as in C).
type RuleLine<'a> = Result<Option<(&'a [u8], Vec<u8>)>, i32>;
...
fn parse_rule_line(line: &[u8]) -> RuleLine<'_> {
    let mut state: i32 = 0;
